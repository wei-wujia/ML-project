## ML-project 机器学习算法框架
# 对外经济贸易大学 AIT209-机器学习与项目实践 课程项目文件
...
## ✨ 项目特点
1. 所有算法基于 NumPy 从零实现
2. 所有算法遵循相同的 `fit(X, y)` / `predict(X)` / `score(X, y)` 接口
3. 多任务支持：支持回归（房价预测）、二分类（Titanic 生存预测）和多分类（MNIST/CIFAR-10 图像分类）
4. 自动工程：自动创建文件夹、保存模型和标准化器、生成训练日志和损失曲线
5. 灵活配置：通过命令行参数自由调整算法超参数，无需修改代码
